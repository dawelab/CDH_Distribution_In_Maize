...
MISS <- read.table("/Volumes/Transcend/Tassel_TagTaxaDist_AllData_v5_v_B73-Ab10HIFI_B-Chrom_v2.Sub1Perc.Missing.txt", header = TRUE)
...
write.csv(DF, file="Tassel_TagTaxaDist_AllData_v2_v_B73-Ab10HIFI_B-Chrom_v2.Sub1Perc.MissingAll.txt", row.names = FALSE, quote = FALSE)
...
for(i in 1:length(SAMPS)) {
  x <- SAMPS[i]
  index <- which(DF_RN$ShortName %in% x)
...
png(filename = "MissingDataByDataSource.png", height = 700, width = 500)
...
png(filename = "MissingDataByAb10Status.png", height = 1100, width = 500)
...
png(filename = "MissingDataByBChromStatus.png", height = 1100, width = 500)
ggplot(data=DF_GROUPS_v2) +
  geom_histogram(aes(x=Perc_Missing, fill=B_Chrom_Status), bins = 100) +
  facet_wrap(~B_Chrom_Status, scales="fixed", ncol = 1) +
  xlab("Proportion of Missing Data By BChrom Status") +
  theme(plot.title = element_text(hjust = 0.5, size = 20), axis.title = element_text(size = 15), axis.text = element_text(size = 15), legend.position = "none")
dev.off()

#I am determining the lowest threshold I can reasonably keep by taking the lowest proportion of missing data and subtracting 0.001. I selected this because it seems reasonable
...
DF_GROUPS_v3_RM <- subset(DF_GROUPS_v3, Data_Source == "Romero-Navarro_etal_2017")

UMSUM <- function(x){
  IN <- which(DF_RN$ShortName %in% x)
  NEW <- DF_RN[IN,]
  DF_GROUPS_v3_OTH <<- rbind(DF_GROUPS_v3_OTH, NEW)
}

lapply(DF_GROUPS_v3_RM$ShortName, UMSUM)


#This writes out the samples that PASSED the filter with their original names
write.csv(DF_GROUPS_v3_OTH, file="SamplesPASSEDMissingDataFILTER.csv", row.names = FALSE, quote=FALSE)

#This plots the data after the cutoff
png(filename = "FilteredMissingDataByDataSource.png", height = 700, width = 600)
...
write.csv(DF_GROUPS_v4, file="SamplesRemovedForExcessMissingData.csv", row.names = FALSE, quote = FALSE)

DF_GROUPS_v5 <- subset(DF_GROUPS_v4, Maize_Type != "Blank")

#63 samples that were not blanks were dropped for low coverage
