...
MERGE_BChrom_RPM_FILT_1 <- MERGE_BChrom_RPM[,KEEP]

#This removes any tag with a MAPQ less than 20
MERGE_BChrom_RPM_FILT_2 <- subset(MERGE_BChrom_RPM_FILT_1, MAPQ >= 20)

#this adds a bin value to each line in the MERGE_BChrom_RPM file 
MERGE_BChrom_RPM_FILT_2$bin <- NA
i=1
x=1
for (i in 1:nrow(MERGE_BChrom_RPM_FILT_2)) {
  print(i)
  for (x in 1:nrow(BINS)) {
    BIN_NUM <- BINS[x,4][[1]]
    START <- BINS[x,2][[1]]
    END <- BINS[x,3][[1]]
    if (MERGE_BChrom_RPM_FILT_2[i,3] > START & MERGE_BChrom_RPM_FILT_2[i,3] < END) {
      MERGE_BChrom_RPM_FILT_2[i,ncol(MERGE_BChrom_RPM_FILT_2)] <- BIN_NUM
    }
  }
}

#This generates a data frame with of all the line names found in the data sest
NAME <- as.data.frame(colnames(MERGE_BChrom_RPM_FILT_2[-c(1:7)]))
colnames(NAME) <- "Name"
...
col.num <- which(colnames(MERGE_BChrom_RPM_FILT_2) %in% DF_OTH$OG_Name)
#This appends the meta data columns to this list
add <- c(1:6,ncol(MERGE_BChrom_RPM_FILT_2))
col.num <- append(add, col.num)
#This selects all of these columns
MERGE_BChrom_RPM_FILT_2_OTH <- MERGE_BChrom_RPM_FILT_2[,c(col.num)]
...
for(i in 1:length(SAMPS)) {
  ...
  INDEX2 <- which(colnames(MERGE_BChrom_RPM_FILT_2) %in% OGNAME)
  TEMP1 <- as.data.frame(MERGE_BChrom_RPM_FILT_2[,INDEX2], drop = FALSE)
  MEANS <- as.data.frame(rowMeans(TEMP1))
  CLEAN_NAME <- NAME_RN[INDEX, "Name"][1]
  colnames(MEANS) <- CLEAN_NAME
  MERGE_BChrom_RPM_FILT_2_OTH <<- cbind(MERGE_BChrom_RPM_FILT_2_OTH, MEANS)
}

MERGE_BChrom_RPM_FILT_2_FIX <- MERGE_BChrom_RPM_FILT_2_OTH

#This writes out the final data frame
fwrite(MERGE_BChrom_RPM_FILT_2_FIX, file="BWAaln_All_v_Ab10HIFIBChrom.BChrom.RPM.RNMean.table", row.names = FALSE)
