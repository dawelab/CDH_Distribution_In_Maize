...
setwd("/scratch/mjb51923/Ab10_Global_Survey/out/AlignGBS_HiFiAb10Corrected_v2/BChrom_Model/Experimental_BChromModel_High")
...
#This defines low copy b chromosme and removes them from this sample
...
for(j in 1:5) {
  ...
  for(i in 1:q) {
    ...
    #write.table(heat_data_samp, file = paste("BChrom_Exp_heat_data_samp_High", IT, i, "txt", sep ="."), row.names = FALSE, quote = FALSE, sep = "\t")
    write.table(heat_data_samp, file = paste("SubSamples/BChrom_Exp_heat_data_samp_High", IT, i, "txt", sep ="."), row.names = FALSE, quote = FALSE, sep = "\t")
  ...
  k=1
  while(k <= q) {
    heat_data_samp <- read.table(paste("SubSamples/BChrom_Exp_heat_data_samp_High", k, "txt", sep ="."), header = TRUE,  sep = "\t", check.names=FALSE)
    #heat_data_samp <- read.table(paste("BChrom_Exp_heat_data_samp_High", IT, k, "txt", sep ="."), header = TRUE,  sep = "\t", check.names=FALSE)
    ...
    if(sum(KMeans_Groups_4$Correct) == length(KMeans_Groups_4$Correct)) {
      ...
      #This rescales the data only for plotting, it does not affect clustering
      heat_data_samp <- MinMax(heat_data_samp)
      
      #pdf(file = paste("Experimental_BChrom_HeatMap_High", IT, j, k,"pdf", sep="."))
...
fwrite(Final, file = paste("KmeansGroups_Files/Kmeans_Exp_Groups_High", IT, "table", sep="."))

fwrite(QC_Controls, file = paste("QualityControl/QualityControl_KMeansGroupingControls_High", IT, "table", sep="."))
