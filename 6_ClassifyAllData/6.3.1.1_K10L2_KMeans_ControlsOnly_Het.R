...
DF_EXTRA_2 <- subset(DF_EXTRA, (Data_Source == "Dawe_Lab_1" | Data_Source == "Dawe_Lab_2") & Ab10_Status == "N10" | Ab10_Status == "K10L2" & Name != "W23_AB10-I.11.DC1"& Name != "W23_AB10-I.13.DC1" & Name != "W23_AB10-II.36.DC1" & Name != "W23_N10.14.DC1")
...
#This rescales
heat_data_control <- MinMax(heat_data_control)
...
pdf(file = paste("K10L2_Controls_HeatMap_AllWarD", "pdf", sep="."))
...
group = kmeans(t(heat_data_K10L2), centers = 2)$cluster

pdf(file = paste("K10L2_Controls_HeatMap_AllKMeans", "pdf", sep="."))
a <- Heatmap(as.matrix(heat_data_K10L2),
             #plotting
             col = grid_color,
             column_title = "Tag Index Across Ab10 Haplotype",
...
HOM <- c("B73_K10L2.10.DC2", "B73_K10L2.4.DC2")
...
for(j in 1:100) {
  ...
  for(i in 1:3) {
    ...
    heat_data_samp <- heat_data_control[,c(TEMP_N10$Name[samp_N10], TEMP_K10L2$Name[samp_K10L2])]
  ...
  for(k in 1:3) {
    ...
    #KMeans_Groups_3$Ab10_Status <-  gsub("Ab10-III", "K10L2", KMeans_Groups_3$Ab10_Status)
    #KMeans_Groups_3$Ab10_Status <-  gsub("Ab10-II", "K10L2", KMeans_Groups_3$Ab10_Status)
    #KMeans_Groups_3$Ab10_Status <-  gsub("Ab10-I", "K10L2", KMeans_Groups_3$Ab10_Status)
    ...
    KMeans_Groups_3$Correct <- ifelse(KMeans_Groups_3$Group == KMeans_Groups_3$Ab10_Status, 1, 0)
  ...
  Final_2 <<- merge(Final_2, KMeans_Groups_All_v2[,c(1,4)], by="Name")
}
