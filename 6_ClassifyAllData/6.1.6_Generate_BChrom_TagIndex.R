...
#This is from 3.4
MERGE_BChrom_RPM_FILT_2_FIX <- vroom::vroom("BWAaln_All_v_Ab10HIFIBChrom.BChrom.RPM.RNMean.table")
...
A_SUM = as.data.frame(apply(MERGE_BChrom_RPM_FILT_2_FIX[,-c(1:7)], 2, function(xx) { by(xx, MERGE_BChrom_RPM_FILT_2_FIX$bin, sum)  }))

#This writes out the sumed file
fwrite(A_SUM, file ="BChrom_TagSums_AllSamples.csv")

######## Tag Density
#This sums the number of tag numbers across bins, but excludes the bin column from the summing

A_DENS = as.data.frame(apply(MERGE_BChrom_RPM_FILT_2_FIX[,-c(1:7)], 2, function(xx) { by(xx, MERGE_BChrom_RPM_FILT_2_FIX$bin, function(yy) { sum(yy > 0) }) }))


#This writes out the tag density file
fwrite(A_DENS, file ="BChrom_TagDens_AllSamples.csv")
...
fwrite(B_COMB, file ="BChrom_TagSumPlusTagDensAllSamples.csv")
