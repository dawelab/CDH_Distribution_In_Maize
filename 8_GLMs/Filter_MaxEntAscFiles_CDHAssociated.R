...
#Load the shapefile with the extent
EXT <- read_sf("/scratch/mjb51923/Ab10_Global_Survey/out/WorldClim2/NoAntarctica.shp")

#This filters the raster to only include values greater than the threshold
r[r[] < threshold ] = NA

#This crops the extend to exclude antarctica where Maize cannot grow
r <- crop(x = r, y = EXT)
...
table <- read.csv("/scratch/mjb51923/Ab10_Global_Survey/Ab10-Global-Survey/7_Map/Stand7_Area_Table.csv")
...
write.csv(table, "/scratch/mjb51923/Ab10_Global_Survey/Ab10-Global-Survey/7_Map/Stand7_Area_Table.csv", row.names=FALSE)
