...
vcfR <- read.vcfR("/Volumes/Transcend/AllData_v1_Ab10Spec.recode.Higgins.vcf")
...
#This applys a filer 
vcfR<-hard_filter(vcfR=vcfR, depth = 3, gq = 70)
...
vcfR<-max_depth(vcfR, maxdepth = 40)
...
miss<-assess_missing_data_pca(vcfR=vcfR, popmap = popmap, thresholds = c(.5,.55,.6), clustering = FALSE)

#This filters the data to remove SNPs with more than 50% missing data
vcfR<-missing_by_snp(vcfR, cutoff = .5)
...
vcfR::write.vcf(vcfR, "/Volumes/Transcend/AllData_v1_Ab10Spec.recode.Higgins.filt.vcf")


#These pavkages can help calculate FST
#https://www.rdocumentation.org/packages/StAMPP/versions/1.6.3/topics/stamppConvert
