...
plink --bfile $DIR/$NAME --allow-extra-chr --pca 10 'header' 'tabs' 'var-wts' --out $DIR/BChr_PCA

#This performs a logistic GWAS for K10L2. --1 indicates my phenotype are coded as 0 and 1
plink --bfile $DIR/$NAME --logistic --beta -covar $DIR/BChr_PCA.eigenvec --pheno $DIR/BChromCall_ForPLINK.txt --1 --allow-no-sex --out BChrom_results_PCA
