...
#This filters the haplotypes to only the Ab10 shared regions 
VCF1 <- subset(VCF, (POS < 142472000))
VCF2 <- subset(VCF, POS > 152050000 & POS < 156880000)
VCF3 <- subset(VCF, POS > 158250000 & POS < 167721000)
...
VCF <- VCF[,c("Sample", "Pop", "Ploidy", "Format", 1:449)]
#This renames the SNPs to reflect their position
colnames(VCF) <- c("Sample", "Pop", "Ploidy", "Format", ID)

#This writes out the file
write.table(VCF, file="/Volumes/Transcend/AllData_v5.Ab10Hap.HigControls.filt.AB.Ab10Shared.vcf", quote=FALSE, row.names = FALSE)
