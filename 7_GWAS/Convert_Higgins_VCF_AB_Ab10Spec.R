...
#This filters the haplotypes to only the Ab10 specific regions 
VCF1 <- subset(VCF, POS > 146699300 & POS < 150656000)
VCF2 <- subset(VCF, POS > 167721000)
VCF <- rbind(VCF1, VCF2)
...
VCF <- VCF[,c("Sample", "Pop", "Ploidy", "Format", 1:123)]
#This renames the SNPs to reflect their position
colnames(VCF) <- c("Sample", "Pop", "Ploidy", "Format", ID)

#This writes out the file
write.table(VCF, file="/Volumes/Transcend/AllData_v5.Ab10Hap.HigControls.filt.AB.Ab10Spec.vcf", quote=FALSE, row.names = FALSE)
