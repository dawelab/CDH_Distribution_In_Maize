...
plink --bfile $DIR/$NAME --logistic --beta -covar $DIR/plink.eigenvec --pheno $DIR/Ab10Call_ForPLINK.txt --1 --allow-no-sex --out Ab10_results_PCA_TE

#This performs a logistic GWAS for K10L2. --1 indicates my phenotype are coded as 0 and 1
plink --bfile $DIR/$NAME --logistic --beta -covar $DIR/plink.eigenvec --pheno $DIR/K10L2Call_ForPLINK.txt --1 --allow-no-sex --out K10L2_results_PCA_TE

#This performs a logistic GWAS for K10L2. --1 indicates my phenotype are coded as 0 and 1
plink --bfile $DIR/$NAME --logistic --beta -covar $DIR/plink.eigenvec --pheno $DIR/BChromCall_ForPLINK.txt --1 --allow-no-sex --out BChrom_results_PCA_TE

#This performs a linear GWAS for BChrom pseudo copy number on only the B chromosome lines. 
plink --bfile $DIR/$NAME --linear --beta --keep $BChromList -covar $DIR/plink.eigenvec --pheno $DIR/CopyBChromCall_ForPLINK.txt --allow-no-sex --out CopyBChrom_results_PCA_TE